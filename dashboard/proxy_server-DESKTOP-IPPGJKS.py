...
def auto_refresh_loop():
    ...
    # ── MANUAL_SESSION 우선 사용
    if MANUAL_SESSION.strip():
        sess.cookies.set("JSESSIONID", MANUAL_SESSION.strip(),
                         domain="www.tempcare.kr", path="/")
        print(f"[{_now()}] [KEY] 브라우저 세션 사용: {MANUAL_SESSION[:8]}...")
        if _verify_api():
            print(f"[{_now()}] [OK] 세션 유효 - 데이터 수집 시작")
            login_ok = True
        else:
            print(f"[{_now()}] [WARN] 세션 만료됨 - 브라우저에서 JSESSIONID를 다시 복사해 MANUAL_SESSION에 붙여넣고 재시작하세요")
            login_ok = False
            return
    else:
        # ── 자동 로그인 시도
        if not do_login():
            print("[!] 자동 로그인 실패.")
            print("    해결방법: 브라우저로 tempcare.kr 로그인 후")
            print("    F12 -> Application -> Cookies -> JSESSIONID 값을")
            print("    proxy_server.py 의 MANUAL_SESSION 에 붙여넣고 재시작하세요.")
            return
...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        original_path = self.path.split("?")[0]
        path = original_path
        ...
        elif path in ("/", "/index.html", "/index1.html"):
            if is_dashboard:
                self._serve_file("index.html")
            else:
                self._serve_root_file("index1.html")
    ...
    def _serve_root_file(self, filename, content_type="text/html; charset=utf-8"):
        import os
        base_dir = os.path.dirname(os.path.abspath(__file__))
