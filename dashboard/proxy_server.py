...
INTERVAL_MIN = 2                    # 데이터 간격(분) - 사이트와 동일하게 2분으로 단축
...
def refresh_cache():
    ...
    if not content:
        print(f"[{_now()}] [WARN] 수집된 데이터 없음 (또는 세션 만료)")
        # 데이터가 없을 때 login_ok를 강제로 False로 만들면 정상적인 빈 데이터 상황(예: 자정 직후)에서도 끊어지게 됩니다.
        # 인증 문제는 fetch_all_pages와 keep_alive_loop에서 처리하므로 여기선 반환만 합니다.
        return
...
# ────────────────────────────────────────────────────
#  세션 유지 백그라운드 스레드 (Ping)
# ────────────────────────────────────────────────────
def keep_alive_loop():
    """세션 만료를 방지하기 위해 주기적으로 가벼운 API 호출을 수행하여 세션을 연장"""
    global login_ok
    while True:
        time.sleep(60)  # 1분마다 핑(ping)으로 세션 만료 절대 방지
        if login_ok:
            try:
                # 1건만 조회하는 가벼운 API 호출로 세션 타임아웃 갱신
                if not _verify_api():
                    print(f"[{_now()}] [WARN] 세션 연장 중 만료 감지됨. 재로그인 시도...")
                    if do_login():
                        login_ok = True
                    else:
                        login_ok = False
                        print(f"[{_now()}] [WARN] 세션을 다시 갱신해주세요.")
            except Exception:
                pass
...
def auto_refresh_loop():
    ...
    while True:
        # 세션이 유효하지 않으면 로그인 복구 시도
        if not login_ok:
            if MANUAL_SESSION.strip():
                sess.cookies.set("JSESSIONID", MANUAL_SESSION.strip(),
                                 domain="www.tempcare.kr", path="/")
                if _verify_api():
                    print(f"[{_now()}] [OK] 세션 유효 - 데이터 수집 시작")
                    login_ok = True
                else:
                    print(f"[{_now()}] [WARN] 세션 만료됨 - 자동 로그인 시도...")
                    if do_login():
                        login_ok = True
                    else:
                        print(f"[{_now()}] [WARN] 세션을 다시 갱신해주세요. 1분 후 재시도...")
            else:
                if do_login():
                    login_ok = True
                else:
                    print(f"[{_now()}] [WARN] 자동 로그인 실패. 1분 후 재시도...")
        ...
        if login_ok:
            ...
            # 다음 갱신까지 대기 (INTERVAL_MIN 분)
            # 도중에 login_ok가 False가 되면 즉시 빠져나와 재로그인
            for _ in range(INTERVAL_MIN * 60):
                if not login_ok:
                    break
                time.sleep(1)
        else:
            time.sleep(60) # 로그인 실패 시 1분 대기 후 재시도
